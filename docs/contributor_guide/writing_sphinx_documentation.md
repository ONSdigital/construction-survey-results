...
Let us say that `construction_survey_results/__init__.py` has functions called `hello` and `world` imported
into it, and both have docstrings. To automatically generate docstring documentation,
create a ReST file, and add the following line to reference the `construction_survey_results` module:

```rest
.. currentmodule:: construction_survey_results
```
...
Let us say that `construction_survey_results/__init__.py` has functions called `hello` and `world` imported
into it, and both have docstrings. To automatically generate docstring documentation,
create a Markdown file, and add the following line to reference the `construction_survey_results` module:
