...
def input_df(filepath):
    return pd.read_csv(filepath / "pounds_thousands_input.csv", index_col=False)


@pytest.fixture(scope="class")
def expected_df(filepath):
    return pd.read_csv(filepath / "pounds_thousands_output.csv", index_col=False)


@pytest.fixture(scope="class")
def test_config():
    return [201, 202, 211, 212, 221, 222, 231, 232, 241, 242, 243, 290]
